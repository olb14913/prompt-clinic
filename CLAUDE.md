...
- [x] F-05-2 — Notion 실패 시 다운로드 버튼 위 fallback 안내 메시지 표시

### F-06 자동아카이빙
- [x] F-06-1 — utils/notion.py `save_diagnosis_page(snapshot)` 구현 완료
- [x] F-06-2 — main.py Notion 저장 버튼 + 1회 재시도 + fallback 연결 완료
...
| 기능 ID | 파일 | 내용 | 상태 |
|---------|------|------|------|
| F-06-1 | utils/notion.py | Notion API 실제 구현 | ✅ 완료 |
| F-06-2 | main.py | Notion 저장 토스트 + 1회 재시도 + fallback 연결 | ✅ 완료 |
| F-05-2 | main.py | F-06-2 실패 시 md 다운로드 버튼 자동 활성화 연동 | ✅ 완료 |
...
karpathy/autoresearch 패턴을 Prompt Clinic에 적용.
- 원리: 프롬프트 개선 → GPT-4o 진단 → total_score 측정 →
  점수 오르면 keep, 안 오르면 다른 전략 시도 → N회 반복 후 최고점 반환
- total_score(0~100)를 autoresearch의 val_bpb처럼 최적화 지표로 활용
- _run_diagnosis의 lc_chat_history가 이 루프에서 활용 예정
  (코드에 주석 표시됨)
- 구현 위치: 새 파일 chains/self_improve_chain.py + main.py 연동

### Few-shot 사례 Notion 연동 자동 갱신
현재 data/fewshot_examples.json은 수동 관리 중.
- Notion DB에 쌓인 Before/After 데이터 활용
- 점수 높은 After → 좋은 사례 풀 자동 추가
- 점수 낮은 Before → 나쁜 사례 풀 자동 추가
- load_fewshot_examples()가 JSON 대신 Notion DB에서 동적으로 읽도록 확장
- F-06 Notion 연동 안정화 후 진행

### 프롬프트 4단계 레벨 시스템
프롬프트 엔지니어링 수준을 4단계로 분류해 사용자 학습 경로 제공.
- 1단계 초급: 단순 요청 수준
- 2단계 중급: 목적 + 조건 + 형식 포함
- 3단계 고급: 맥락 유지 + 단계적 지시 + 비교 요청
- 4단계 전문가: Few-shot / CoT / Role Prompting 기법 활용
- 현재 total_score와 연계해 사용자 레벨 자동 판정
- 레벨별 개선 가이드 및 템플릿 라이브러리 제공
- 누적 히스토리에서 패턴 추출 → 레벨별 템플릿 자동 생성
- 구현 시 Opus 모델 활용 권장 (복잡한 판단 기준 설계 필요)
